(function () {
    ...
    var originConfig = {
        europe: { label: 'Європа', baseFlight: 1080, note: '~1000 EUR / людина' },
        'usa-ny': { label: 'США - Нью-Йорк', baseFlight: 480, note: '~$480 / людина' },
        'usa-miami': { label: 'США - Маямі', baseFlight: 360, note: '~$360 / людина' },
        'usa-la': { label: 'США - Лос-Анджелес', baseFlight: 560, note: '~$560 / людина' },
        canada: { label: 'Канада', baseFlight: 650, note: '~C$700 / людина' },
        other: { label: 'Інший напрямок', baseFlight: 600, note: '~$600 / людина' }
    };
    var formatConfig = {
    ...
    var zoneConfig = {
        north: { label: 'Hotel Zone - північ', multiplier: 1.20 },
    ...
    var leadTimeConfig = [
        { minDays: 0, maxDays: 14, flight: 1.35, hotel: 1.12, label: 'дуже близько до вильоту' },
        { minDays: 15, maxDays: 45, flight: 1.15, hotel: 1.06, label: 'близька дата вильоту' },
        { minDays: 46, maxDays: 90, flight: 1.00, hotel: 1.00, label: 'нормальне вікно бронювання' },
        { minDays: 91, maxDays: 180, flight: 0.93, hotel: 0.97, label: 'раннє бронювання' },
        { minDays: 181, maxDays: 9999, flight: 0.96, hotel: 1.00, label: 'дуже рання оцінка' }
    ];
    ...
    var hotelCatalog = [
        { name: 'Antillano Hotel', zone: 'downtown', formats: ['budget'], priceMin: 45, priceMax: 72, directUrl: 'https://www.booking.com/searchresults.html?ss=Antillano%20Hotel%20Cancun', tags: ['budget', 'центр'] },
        { name: 'Hotel Plaza Kokai', zone: 'downtown', formats: ['budget', 'comfort'], priceMin: 58, priceMax: 86, directUrl: 'https://www.booking.com/searchresults.html?ss=Hotel%20Plaza%20Kokai%20Cancun', tags: ['ринок', 'транспорт'] },
        { name: 'Riu Caribe', zone: 'center', formats: ['comfort', 'ai'], priceMin: 140, priceMax: 235, directUrl: 'https://www.booking.com/searchresults.html?ss=Riu%20Caribe%20Cancun', tags: ['пляж', 'баланс'] },
        { name: 'The Westin Cancun', zone: 'center', formats: ['comfort'], priceMin: 150, priceMax: 210, directUrl: 'https://www.booking.com/searchresults.html?ss=The%20Westin%20Cancun', tags: ['сервіс', 'пляж'] },
        { name: 'Grand Fiesta Americana', zone: 'north', formats: ['comfort', 'ai'], priceMin: 165, priceMax: 250, directUrl: 'https://www.booking.com/searchresults.html?ss=Grand%20Fiesta%20Americana%20Cancun', tags: ['північ', 'сильний пляж'] },
        { name: 'Krystal Grand Cancun', zone: 'north', formats: ['comfort'], priceMin: 145, priceMax: 205, directUrl: 'https://www.booking.com/searchresults.html?ss=Krystal%20Grand%20Cancun', tags: ['сімейний'] },
        ...
        { name: 'Moon Palace Cancun', zone: 'center', formats: ['ai'], priceMin: 290, priceMax: 420, directUrl: 'https://www.booking.com/searchresults.html?ss=Moon%20Palace%20Cancun', tags: ['великий resort'] },
        { name: 'Hard Rock Hotel Cancun', zone: 'center', formats: ['ai'], priceMin: 260, priceMax: 390, directUrl: 'https://www.booking.com/searchresults.html?ss=Hard%20Rock%20Hotel%20Cancun', tags: ['активний'] }
    ];
    var el = {
    ...
    function nightWord(n) { var d = n % 10, h = n % 100; if (d === 1 && h !== 11) return 'ніч'; if (d >= 2 && d <= 4 && (h < 12 || h > 14)) return 'ночі'; return 'ночей'; }
    function personWord(n) { var d = n % 10, h = n % 100; if (d === 1 && h !== 11) return 'людина'; if (d >= 2 && d <= 4 && (h < 12 || h > 14)) return 'людини'; return 'людей'; }
    function roomWord(n) { var d = n % 10, h = n % 100; if (d === 1 && h !== 11) return 'номер'; if (d >= 2 && d <= 4 && (h < 12 || h > 14)) return 'номери'; return 'номерів'; }
    function dayWord(n) { var d = n % 10, h = n % 100; if (d === 1 && h !== 11) return 'день'; if (d >= 2 && d <= 4 && (h < 12 || h > 14)) return 'дні'; return 'днів'; }
    function formatUaDate(d) { var m = ['січня', 'лютого', 'березня', 'квітня', 'травня', 'червня', 'липня', 'серпня', 'вересня', 'жовтня', 'листопада', 'грудня']; return d.getDate() + ' ' + m[d.getMonth()] + ' ' + d.getFullYear(); }
    function seasonLabel(k) { return k === 'high' ? 'високий сезон' : (k === 'low' ? 'низький сезон' : 'середній сезон'); }
    ...
    function statusForBudget(budget, s) {
        if (budget >= s.totalReal + 700) return { key: 'fit', label: 'Є запас' };
        if (budget >= s.totalReal) return { key: 'tight', label: 'Впритул' };
        if (budget >= s.totalMin) return { key: 'over', label: 'Вище бюджету' };
        return { key: 'low', label: 'Потрібно більше' };
    }
    ...
    function renderOptions(options, budget, primaryKey) {
        ...
        options.forEach(function (s) {
            ...
            card.children[1].textContent = 'Реалістично: ' + usd(s.totalReal);
            card.children[2].textContent = 'Діапазон: ' + usd(s.totalMin) + ' - ' + usd(s.totalMax) + ' · ' + scenarioMeta(s);
            card.children[3].textContent = budget >= s.totalReal ? 'Вписується, запас ' + usd(budget - s.totalReal) : 'Не вистачає ' + usd(s.totalReal - budget);
    ...
    function renderSavings(current, input, ctx, extrasState) {
        ...
        if (input.nights >= 4) add('Прибрати 1 ніч', 'Наймʼякший спосіб знизити підсумок.', buildScenario({ origin: input.origin, format: input.format, zone: input.zone, nights: input.nights - 1, people: input.people }, ctx, extrasState));
        if (input.zone === 'north') add('Перейти в центральну Hotel Zone', 'Пляжна логіка зберігається, ціна нижча.', buildScenario({ origin: input.origin, format: input.format, zone: 'center', nights: input.nights, people: input.people }, ctx, extrasState));
        if (input.zone === 'center') add('Перейти в Downtown', 'Найсильніший важіль економії, але пляж стане окремою поїздкою.', buildScenario({ origin: input.origin, format: input.format, zone: 'downtown', nights: input.nights, people: input.people }, ctx, extrasState));
        if (input.format === 'ai') add('Змінити all inclusive на Comfort', 'Корисно, якщо плануєш багато виїздів.', buildScenario({ origin: input.origin, format: 'comfort', zone: input.zone, nights: input.nights, people: input.people }, ctx, extrasState));
        if (input.format === 'comfort') add('Змінити Comfort на Budget', 'Працює для самостійної поїздки.', buildScenario({ origin: input.origin, format: 'budget', zone: input.zone, nights: input.nights, people: input.people }, ctx, extrasState));
        tips.sort(function (a, b) { return b.saving - a.saving; });
        if (!tips.length) tips.push({ title: 'Сценарій уже близький до оптимального', detail: 'Залиши невеликий запас на зміну цін.', saving: 0 });
        tips.slice(0, 4).forEach(function (tip) {
            ...
            if (tip.saving > 0) {
                ...
                amt.textContent = 'Економія близько ' + usd(tip.saving);
    ...
    function buildExplainText(s) {
        ...
        var text = 'Виліт: ' + s.originLabel + '. До поїздки ' + s.context.daysUntilDeparture + ' ' + dayWord(s.context.daysUntilDeparture) + ' (' + s.context.lead.label + '), період: ' + seasonLabel(s.context.season.season) + '. ';
        text += 'Для ' + s.people + ' ' + personWord(s.people) + ' житло рахується на ' + s.rooms + ' ' + roomWord(s.rooms) + '. ';
        text += extrasCount > 0 ? 'Додаткові витрати включені в підсумок.' : 'Додаткові витрати не включені, це базова оцінка.';
        return text;
    }
    ...
    function bookingSearchUrl(s) {
        ...
        return 'https://www.booking.com/searchresults.html?ss=' + encodeURIComponent(destinationConfig.displayName + ' ' + s.zoneLabel) + '&checkin=' + encodeURIComponent(inDate) + '&checkout=' + encodeURIComponent(outDate) + '&group_adults=' + encodeURIComponent(String(s.people)) + '&no_rooms=' + encodeURIComponent(String(s.rooms)) + '&selected_currency=USD';
    }
    function renderHotelCards(s) {
        ...
        pickHotels(s.formatKey, s.zoneKey).forEach(function (h) {
            ...
            card.innerHTML = '<div class="chc-name"></div><div class="chc-price"></div><div class="chc-tag"></div><div class="chc-link-hint">Відкрити на Booking</div>';
            card.children[0].textContent = h.name;
            card.children[1].textContent = usd(h.priceMin) + ' - ' + usd(h.priceMax) + ' / ніч';
    ...
    function renderUrgency(s) {
        if (s.context.daysUntilDeparture <= 14) {
            el.urgTitle.textContent = 'Поїздка близько, ціни можуть рости щодня';
            el.urgText.textContent = 'У такому вікні перельоти і хороші готелі зазвичай дорожчають швидше.';
        } else if (s.context.season.season === 'high') {
            el.urgTitle.textContent = 'Ти вибрав високий сезон Канкуна';
            el.urgText.textContent = 'У пікові місяці хороші варіанти закінчуються раніше.';
        } else {
            el.urgTitle.textContent = 'Спокійне вікно для бронювання';
            el.urgText.textContent = 'Зараз зазвичай простіше зловити хороший баланс ціни та якості.';
        }
    }
    function renderScenario(current, options, budget) {
        ...
        el.meta.textContent = current.zoneLabel + ' · ' + scenarioMeta(current) + ' · виліт ' + formatDateInput(current.context.departureDate);
        el.breakdown.innerHTML = 'Переліт: ' + usd(current.flight.real) + ' <span class="calc2-inline-note">(' + current.originNote + ')</span> · Готель: ' + usd(current.hotel.real) + ' · Інші витрати: ' + usd(current.daily.real) + (current.extras.real > 0 ? ' · Додатково: ' + usd(current.extras.real) : '') + '<br>Орієнтир в EUR: ' + eur(current.totalReal * destinationConfig.defaultEurRate);
        ...
        if (status.key === 'fit') el.budgetSummary.textContent = 'Є запас близько ' + usd(budget - current.totalReal) + '. Цей сценарій можна залишити.';
        else if (status.key === 'tight') el.budgetSummary.textContent = 'Сценарій майже точно в бюджеті. Запас близько ' + usd(budget - current.totalReal) + '.';
        else if (status.key === 'over') el.budgetSummary.textContent = 'Реалістична оцінка вище бюджету на ' + usd(current.totalReal - budget) + ', але нижній діапазон ще можливий.';
        else el.budgetSummary.textContent = 'Навіть нижня оцінка вище бюджету на ' + usd(current.totalMin - budget) + '. Потрібен економніший сценарій.';
    ...
    function calc() {
        ...
        el.today.innerHTML = '<strong>Сьогодні:</strong> ' + formatUaDate(ctx.today);
